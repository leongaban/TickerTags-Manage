...
function ReportedTagsCtrl (
    ...
    const massCategory = () => {
        TagsFactory.massChangeCategory();
    };

    const changeCategory = (tag) => {
        TagsFactory.changeCategory(tag, tag.ticker);
    };

    const reportApprove = (report) => {
        TagsFactory.approveTag(report, 'tagReports');
    };

    const reportDeny = (report) => {
        TagsFactory.denyTag(report);
    };
